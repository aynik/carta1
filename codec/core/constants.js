...
export const BFU_TO_BAND = (() => {
  const table = new Uint8Array(52)
  for (let i = 0; i < BFU_BAND_BOUNDARIES[0]; i++) table[i] = 0
  for (let i = BFU_BAND_BOUNDARIES[0]; i < BFU_BAND_BOUNDARIES[1]; i++)
    table[i] = 1
  for (let i = BFU_BAND_BOUNDARIES[1]; i < BFU_BAND_BOUNDARIES[2]; i++)
    table[i] = 2
  return table
})()

// Perceptually important bfus
export const CRITICAL_BFUS = new Uint8Array([
  20, 21, 22, 19, 23, 18, 24, 17, 25, 16, 26, 15, 27, 14, 28, 32, 33, 34, 35,
  36, 37, 38, 31, 30, 29,
])
...
// Band sizes
export const BAND_SIZE_LOW = 128
export const BAND_SIZE_MID = 128
export const BAND_SIZE_HIGH = 256

// Window positions
export const WINDOW_START_LONG_HIGH = 112
export const WINDOW_START_LONG_MID = 48
export const WINDOW_START_SHORT = 0

// IMDCT overlap sizes
export const IMDCT_OVERLAP_SIZE = 16
export const IMDCT_QUARTER_SIZE = 4
export const IMDCT_SHORT_BLOCK_SIZE = 32
...
export const ATH_LINEAR = (() => {
  const table = new Float32Array(NUM_BFUS)
  for (let i = 0; i < NUM_BFUS; i++) {
    const freqKhz = BFU_FREQUENCIES[i] / 1000
    const athDb =
      3.64 * Math.pow(freqKhz, -0.8) -
      6.5 * Math.exp(-0.6 * Math.pow(freqKhz - 3.3, 2)) +
      1e-3 * Math.pow(freqKhz, 4)
    table[i] = Math.pow(10, Math.max(-10, Math.min(96, athDb)) / 10)
  }
  return table
})()
...
export const PSYMODEL_INPUT_SIZE = 512
export const PSYMODEL_PN = 90.302

// Transient detection
export const TRANSIENT_DETECTION_THRESHOLD = 0.1
export const TRANSIENT_FLATNESS_EPSILON = 1e-10
...
// Bit allocation
export const ALLOCATION_DIMINISHING_RETURNS_THRESHOLD = 0.1
export const ALLOCATION_MAX_ITERATIONS = 100

// Quantization
export const QUANTIZATION_SIGN_BIT_SHIFT = 1
export const QUANTIZATION_MAX_RANGE_SHIFT = 1

// WAV format
export const WAV_HEADER_SIZE = 44
export const WAV_RIFF_CHUNK_ID = 0x46464952
export const WAV_WAVE_FORMAT = 0x45564157
export const WAV_FMT_CHUNK_ID = 0x20746d66
export const WAV_DATA_CHUNK_ID = 0x61746164
...
// Display formatting
export const DISPLAY_HEADER_WIDTH = 64
export const DISPLAY_FRAME_WIDTH = 32
export const DISPLAY_FORMAT_WIDTH = 16
...
// Hann window for 512 samples (single-band psychoacoustics)
export const PSYMODEL_HANN_WINDOW = (() => {
  const table = new Float32Array(512)
  const normalizationFactor = Math.sqrt(8.0 / 3.0)

  for (let i = 0; i < 512; i++) {
    const hann = 0.5 * (1 - Math.cos((2 * Math.PI * i) / 512))
    const normalized = hann * normalizationFactor
    table[i] = Math.round(normalized * 10000) / 10000
  }
  return table
})()
