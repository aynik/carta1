...
// RDO bit allocation
export const LP_DISTORTION_P = 4
export const LP_DISTORTION_CONSTANT = 1 / 80 // For L4 (p=4), it's 1/80.
export const { POW2_NEG_P, POW2_POS_P, SF_POW_P } = (() => {
  // Calculate the 2^{-p*b} table
  const maxBits = WORD_LENGTH_BITS[MAX_WORD_LENGTH_INDEX]
  const pow2_neg_p = new Float64Array(maxBits + 1)
  for (let b = 0; b <= maxBits; b++) {
    pow2_neg_p[b] = Math.pow(2, -LP_DISTORTION_P * b)
  }

  // Calculate the 2^{p} constant for the D(0) case
  const pow2_pos_p = Math.pow(2, LP_DISTORTION_P)

  // Calculate the (scale_factor)^p table
  const sfPowP = new Float64Array(SCALE_FACTORS.length)
  for (let i = 0; i < SCALE_FACTORS.length; i++) {
    sfPowP[i] = Math.pow(SCALE_FACTORS[i], LP_DISTORTION_P)
  }

  // Return all computed tables in a single object
  return { POW2_NEG_P: pow2_neg_p, POW2_POS_P: pow2_pos_p, SF_POW_P: sfPowP }
})()
